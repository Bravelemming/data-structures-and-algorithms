...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach(element => {
    //i no longer exists within the for loop, so we must use...
    let temp = Math.pow(base, element);
    resultArr.push(temp);
  });
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let base = 2;
  var mapReturn = arr.map(function (element){
    return Math.pow(base, element);
  })
