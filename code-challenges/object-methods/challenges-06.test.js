...
const houseSize = (arr) => {
  ...
  arr.forEach( (el) =>{
    let foo = [];
    foo.push(el);
    let ccount = totalCharacters(foo);
    sizes.push({'house':el.house, 'members': ccount});
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(el => {
    ...
    if (deceasedSpouses.includes(el.spouse) === false) {
      if (el.spouse !== null) {
        totalCharacters++;
      }
    }
